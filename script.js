...
function randomWord() {
  const randonIndex = Math.ceil(Math.random() * wordsCount);
  return words[randonIndex - 1];
}// ✅
...
function gameOver() {
  ...
  const result = getWpm();
  document.getElementById('wpm').innerHTML = result;
} // ✅

document.querySelector("#game").addEventListener("keyup", e => {
  const key = e.key;
